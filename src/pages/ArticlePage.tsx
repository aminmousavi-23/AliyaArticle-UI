...
import { countWords, estimateReadTime, formatDate, initials } from '@/lib/format'
...
export function ArticlePage() {
  ...
  async function handlePostComment(values: { authorName: string; authorEmail: string; content: string }) {
    ...
    try {
      ...
      showToast(getApiErrorMessage(err, 'Could not post your comment.'), 'error')
    } finally {
      setIsPosting(false)
    }
  }
  ...
  async function handleDeleteComment(commentId: string) {
    try {
      ...
      showToast(getApiErrorMessage(err, 'Could not delete that comment.'), 'error')
    } finally {
  ...
  if (isLoading) return <div className="page"><Spinner label="Loading article" /></div>
    ...
    return (
      <div className="page">
        <div className="container">
          ...
          <Link className="btn btn--primary" to="/">Back to articles</Link>
        </div>
      </div>
    )
  }
              ...
              <div>
                ...
                <div className="meta">
                  ...
                  {!article.isPublished && (
                    <>
                      <span className="meta__dot" />
                      <span className="badge badge--draft">Draft — not yet published</span>
                    </>
                  )}
                </div>
              </div>
